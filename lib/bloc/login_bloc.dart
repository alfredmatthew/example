...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  void _onFormRegisterSubmitted(
    ...
    emit(state.copyWith(formSubmissionState: FormSubmitting()));
    try {
      await registerRepository.register(event.name, event.email, event.password, event.telepon, event.tanggalLahir, event.usia);

      // registerRepository.clearUsers();
      registerRepository.printAllUsers();
      SharedPreferences prefs = await SharedPreferences.getInstance();
      
      prefs.setString('username', event.name);
      prefs.setString('email', event.email);
      prefs.setString('password', event.password);
      prefs.setString('telepon', event.telepon);
      prefs.setString('tanggalLahir', event.tanggalLahir);
      prefs.setString('usia', event.usia); 
